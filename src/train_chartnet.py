...
def augment_mel(mel):
    ...
    if shift != 0:
        mel = torch.roll(mel, shift, dims=2)
    # Gaussian noise
...
def train_epoch(model, loader, optimizer, criterion, device, grad_clip=1.0,
    ...
    for mel, note_t, fret_t, sustain_t, chord_t, type_t, diff_t in pbar:
        ...
        pbar.set_postfix(loss=f'{loss.item():.4f}',
                         note=f'{sub["note"].item():.3f}',
                         fret=f'{sub["fret"].item():.3f}')
...
def main():
    ...
    ap.add_argument('--patience', type=int, default=8,
                    help='Early stopping: stop if val F1 does not improve for N epochs (0 = disabled)')
    ap.add_argument('--reset-best', action='store_true',
                    help='Reset best F1 to 0 on resume (use when dataset changes)')
    ap.add_argument('--reset-lr', action='store_true',
                    help='Reset LR schedule on resume (use when dataset changes)')
    args = ap.parse_args()
    ...
    # ── Model ─────────────────────────────────────────────────────────────
    model_kwargs = dict(n_mels=128, diff_emb_dim=32,
    ...
    # Chord head gets 3x higher LR to recover from collapse faster
    chord_params = list(model.chord_head.parameters())
    chord_ids    = {id(p) for p in chord_params}
    base_params  = [p for p in model.parameters() if id(p) not in chord_ids]
    optimizer = optim.AdamW([
        {'params': base_params,  'lr': args.lr},
        {'params': chord_params, 'lr': args.lr * 3.0, 'name': 'chord_head'},
    ], weight_decay=1e-4)
    ...
    print(f'  Model: ChartNet v3  {n_params/1e6:.1f}M parameters')
    print(f'  Architecture: CNN(5 ResBlocks, 128→768ch) → Transformer(8h,6L,768d) → 5 heads')
    print(f'  Chord head LR: {args.lr * 3.0:.1e}  (3x base={args.lr:.1e})')
    print(f'  Heads: note | fret(5) | sustain(4) | chord(6,weighted) | type(4)')
    print(f'  AMP: {"enabled (fp16)" if use_amp else "disabled (cpu)"}')
    print(f'  LR schedule: {warmup_epochs} epoch warmup → cosine decay to 1e-6')

    start_epoch    = 0
    best_f1        = 0.0
    epochs_no_imp  = 0

    # Auto-resume: pick explicit --resume, else latest epoch_*.pt, else nothing
    resume_path = None
    if args.resume and Path(args.resume).exists():
        resume_path = Path(args.resume)
    else:
        existing = sorted(out_dir.glob('epoch_*.pt'))
        if existing:
            resume_path = existing[-1]

    if resume_path:
        ckpt = torch.load(resume_path, map_location=device, weights_only=True)
        # Load model weights — strict=False allows loading v2 weights into v3
        missing, unexpected = model.load_state_dict(ckpt['model_state'], strict=False)
        if missing:
            print(f'  [resume] New params (random init): {len(missing)}')
        if not args.reset_lr:
            try:
                ...
                print(f'  [resume] Could not load optimizer state ({e}), resetting LR')
        else:
            print(f'  [resume] LR schedule reset (--reset-lr)')
        if use_amp and 'scaler_state' in ckpt:
            scaler.load_state_dict(ckpt['scaler_state'])
        start_epoch   = ckpt.get('epoch', 0) + 1
        best_f1       = 0.0 if args.reset_best else ckpt.get('best_f1', 0.0)
        epochs_no_imp = 0   if args.reset_best else ckpt.get('epochs_no_imp', 0)
        print(f'  Auto-resumed from {resume_path.name}  epoch={start_epoch}  best_f1={best_f1:.3f}{"  (best reset)" if args.reset_best else ""}')
    ...
    for epoch in epoch_bar:
        ...
        is_best = va['note_f1'] > best_f1

        epoch_bar.set_postfix(
            loss=f'{tr["loss"]:.4f}',
            F1=f'{va["note_f1"]:.3f}',
            fret=f'{va["fret_acc"]:.3f}',
            sus=f'{va["sustain_acc"]:.3f}',
            best=f'{max(best_f1, va["note_f1"]):.3f}',
            lr=f'{lr_now:.1e}',
        )
        ...
        tqdm.write(
            ...
            f'loss={tr["loss"]:.4f}  val={va["loss"]:.4f}  '
        ...
        if is_best:
            ...
            tqdm.write(f'  ★  New best F1={best_f1:.3f} → saved best.pt')
        ...
        ckpt_periodic = {
            ...
            'model_kwargs':  model_kwargs,
            'best_f1':       best_f1,
            'epochs_no_imp': 0 if is_best else (epochs_no_imp + 1),
            'val_metrics':   va,
            'train_metrics': tr,
        }
        if use_amp:
        ...
        if is_best:
            ...
            if args.patience > 0 and epochs_no_imp >= args.patience:
                tqdm.write(
                    f'  Early stop: no val F1 improvement for {args.patience} '
                    f'epochs (best={best_f1:.3f})')
                break

        # GPU cooldown break between epochs
        if device.type == 'cuda' and epoch + 1 < args.epochs:
            tqdm.write(f'  [cooldown] 90s GPU break...')
            time.sleep(90)
