...
def build_diff(diff, beats, eighths, sixteenths, onset_ticks,
               ...
               snare_ticks=None, bass_at=None, beat_times=None):
    ...
    # ── ChartNet neural prediction (primary path when model is loaded) ────
    nn_notes = None
    if beat_times is not None:
        nn_notes = chartnet_notes(y, sr, beat_times, tps, diff, solo_tick_set, active_pool)

    if nn_notes is not None:
        print(f'  [{diff}] ChartNet: {len(nn_notes)} notes')
        notes = nn_notes
    else:
...
def generate(mp3_path, fret_pool=None, out_dir='.', no_lyrics=False):
    ...
    for diff in ('expert','hard','medium','easy'):
        notes = build_diff(
            ...
            kick_ticks=kick_ticks, snare_ticks=snare_ticks,
            bass_at=bass_at, beat_times=beat_times)
    ...
    if no_lyrics:
        evs = sorted(sec_events, key=lambda x: x[0])
        events_block = '[Events]\n{\n' + ''.join(f'  {t} = E "{e}"\n' for t,e in evs) + '}'
    else:
...
Examples:
  ...
  python chartgen.py --out "C:/Clone Hero/Songs" song.mp3
  python chartgen.py --frets 0,1,2,3,4 song.mp3        # all 5 frets
  python chartgen.py --frets 0,1,2     song.mp3        # GRY only (default)
...
""")
    ...
    if not songs:
        print('Usage: python chartgen.py [--model checkpoint.pt] [--out ./output] [--frets 0,1,2] <song.mp3> ...')
        sys.exit(1)

    missing = [s for s in songs if not os.path.isfile(s)]
    if missing:
    ...
    for mp3 in song_bar:
        ...
        try:
            ...
            tqdm.write(f'  [error] {os.path.basename(mp3)}: {e}')

    if len(songs) > 1:
        print(f'\nDone: {ok} succeeded, {fail} failed')
        for mp3, err in failed:
            print(f'  ✗ {mp3}: {err}')
